...
Turns out the average number of sixes is `r number_of_sixes_mean`. The easiest way of generating samples is by drawing a random sample from the probability distribution, and if this sample is large enough, each value will occur roughly proportionally often to how probable it is.
...
![marg_hist](readme_files/figure-gfm/marg_hist.gif){width=75%}
...
2. The reason we've called it `prior` is because it represents the uncertainty *before* (that is, *prior* to) having included the information in the data. Let's do that now! 
...
3. Great! The reason that we call it `posterior` is because it represents the uncertainty *after* (that is, *posterior* to) having included the information in the data. So what does `posterior` contain now? Well, `posterior$n_visitors` isn't too exciting, that's just `13` repeated many times. 
...
1. The result from the last exercise is still in the data frame `posterior`, but if you look at `posterior$n_visits` you'll see it's just `13` repeated over and over again. This makes sense as `posterior` represents what the model knew about the outcome of the last ad campaign after having seen the data. 
...
2. Take a look at the first rows in `prior` using the `head()` function.
...
3. While `proportion_clicks` represents the uncertainty regarding the underlying proportion of clicks for the next ad campaign, `n_visitors` has not been updated yet. 
...
4. Here is some example code you can use for this, just replace ___ .
...
5. The plot shows a probability distribution over the number of site visitors a new ad campaign would bring in. It now looks pretty likely that there would be more than, say, 5 visitors because of the campaign. 

    * Calculate the probability that you will get 5 or more visitors next time you run your ad campaign. 
6. Do this by summing up the number of draws with more than five visitors ( `sum(prior$n_visitors >= 5)` ) and divide by the total number of draws ( `length(prior$n_visitors)` ).
...
the prior for the next ad campaign and predicted how many visitors we would get if reran it. I hope you can see that if you collected even more data, you could continue repeating these steps, to learn more and more about the underlying proportion of clicks.This is exactly how
...
2. Right! A Beta(1,1) distribution is the same as a uniform distribution between 0 and 1. It is useful as a so-called *non-informative* prior as it expresses than any value from 0 to 1 is equally likely. 
...
3. Yes, `NaN` stands for *not a number* and the reason you got a lot of `NaN`s is that the Beta distribution is only defined when its shape parameters are positive.

    * Instead, set both shape parameters to a large number. Let's set both `shape1` and `shape2` to `100`. 
...
4. So the larger the shape parameters are, the more concentrated the beta distribution becomes. When used as a prior, this Beta distribution encodes the information that the parameter is most likely close to 0.5 .
...
1. Change this model to use the new informative prior for `proportion_clicks` that you just selected:
...
1. To the right, you again have the model you developed in the last chapter. Here `posterior_video` is the posterior proportion of clicks for the video ad data. 

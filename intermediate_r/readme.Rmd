...
:white_large_square Executing `res <- two_dice()` makes the contents of dice1 and dice2 available outside the function.
...
<pre>`multiply <- function(x, factor) {
  x * factor
}
...
<pre>`lapply(split_low, function(x) {
  if (nchar(x[1]) > 5) {
    return(NULL)
  } else {
    return(x[2])
...
<pre>`sapply(list(runif (10), runif (10)), 
...
<pre>`vapply(X, FUN, FUN.VALUE, ..., USE.NAMES = TRUE)
`<pre>
...
<pre>`birth <- as.POSIXct("1879-03-14 14:37:23")
